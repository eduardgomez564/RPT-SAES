...
export default function Login() {
  ...
  const getPasswordStrength = (pwd: string) => {
    if (pwd.length < 6) return { strength: "Password is weak", color: "text-red-500 font-semibold" };
    if (pwd.length < 10) return { strength: "Password is medium", color: "text-yellow-500 font-semibold" };
            ...
            <div className="mb-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="
                    /* Mobile */
                    w-full px-4 py-2.5 pr-12 border-2 border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#013300] focus:border-transparent transition placeholder-gray-400 text-[#013300]
              ...
              {password && (
                <div className={`flex items-center gap-1 mt-1 text-xs ${getPasswordStrength(password).color}`}>
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    <circle cx="12" cy="12" r="10"/>
                    <path d="M12 16v-4M12 8h.01" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span>{getPasswordStrength(password).strength}</span>
                </div>
              )}
            </div>
